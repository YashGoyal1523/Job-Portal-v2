...
const {titleRef,locationRef,isSearched,searchFilter,setSearchFilter,jobs}=useContext(AppContext)

// to handle viewability of side bar filter according to screen size
const [showFilter,setShowFilter]=useState(false)

//pagination
const [currentPage,setCurrentPage]=useState(1)

//filtering data based on search and filters applied
...
useEffect(()=>{
  ...
  return (
    ...
    <div className='w-full lg:w-1/4 bg-white px-4'>
            ...
            <>
                ...
                (
                    <span className='inline-flex items-center gap-2.5 bg-blue-50 border border-blue-200 px-4 py-1.5 rounded'>
                        {searchFilter.title}
                        <img className=" cursor-pointer" src={assets.cross_icon} onClick={()=>{  setSearchFilter(prev => ( {...prev,title:""})) ; titleRef.current.value=""    }}  />
                    </span>
                )
                }
                ...
                (
                    <span className='ml-2 inline-flex items-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded'>
                        {searchFilter.location}
                     <img className=" cursor-pointer" src={assets.cross_icon} onClick={()=>{  setSearchFilter(prev =>  ({...prev,location:""})) ; locationRef.current.value=""}} />
                    </span>
                )
                }
                </div>
     ...
     {/* viewability of side bar filter */}
